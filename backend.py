...
# Function to create Streamlit logger

class StreamlitLoggerCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        self.placeholder.code(msg)

    def on_train_end(self, logs=None):
        self.placeholder.empty()

# Function to create Embedding Model architecture
...
# Function to prepare dataset for Embedding Models
...
# Function to split dataset into train and validation sets for Embedding Models
...
# Function for generating embedding features

def emb_train(ds_train, ds_val, num_users, num_items, epochs = 10, embedding_size = 16, placeholder=None):
    ...
    callbacks = []
    if placeholder:
        callbacks.append(StreamlitLoggerCallback(placeholder))

    emb_model_hist = emb_model.fit(x = ds_train, validation_data = ds_val, epochs=epochs, verbose=0, callbacks=callbacks)

    return emb_model, emb_model_hist

# Function to train and store Embedding Models

def Embedding_train(model_name, placeholder = None):
    ...
    emb, _ = emb_train(ds_train=ds_train, ds_val=ds_val, num_users=num_users, num_items=num_items, placeholder = placeholder)
...
# Function to recommend courses based on Embedding Models

def Embedding_pred(model_name, user_id, n_rec = 10):
    ...
    r = dict(sorted(result_dict.items(), key=lambda item: item[1], reverse=True))
